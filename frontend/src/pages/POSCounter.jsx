 ...
 * Vista para cajero/recepcionista: menú visual por categorías, carrito lateral,
 * selección de canal (pos | call), nombre del cliente y confirmación de pedido.
 */
...
import {
  ...
  Trash2, Send, X, ChevronRight, Tag, Zap
} from 'lucide-react';
...
export default function POSCounter() {
  ...
  // Modal de nota por item
  const [noteModal, setNoteModal]   = useState(null); // {cartIndex}
  ...
  const handleSubmit = async () => {
    ...
    const confirmed = await showConfirm(
      'Confirmar Pedido',
      `${itemCount} producto(s) — $${total.toFixed(2)}\nCliente: ${clientName}\nCanal: ${channel === 'pos' ? 'Mostrador' : 'Teléfono'}`
    );
    if (!confirmed) return;
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 80px)', gap: '0', overflow: 'hidden' }}>
      ...
      {/* ── Panel derecho: Carrito + Datos del pedido ── */}
